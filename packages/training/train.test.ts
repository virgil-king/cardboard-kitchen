...
test("EpisodeTrainingData: codec round trip", () => {
  ...
  const fromJsonObject = EpisodeTrainingData.decode(
    PickANumber.INSTANCE,
    jsonObject
  );
  const jsonString = JSON.stringify(jsonObject);
  const fromJsonString = EpisodeTrainingData.decode(
    PickANumber.INSTANCE,
  ...
  assert.isTrue(
    fromJsonObject.episodeConfig.players.equals(
      trainingData.episodeConfig.players
    )
  );
  assert.isTrue(
