...
import { HiddenLayerStructure, KingdominoModel } from "./model-linear.js";
...
async function main() {
  ...
  let model: KingdominoConvolutionalModel;
  if (modelPath == undefined) {
    model = KingdominoConvolutionalModel.fresh();
    console.log("Created randomly initialized model");
  } else {
    model = await KingdominoConvolutionalModel.load(modelPath);
  ...
  const modelsDir = `${home}/models/kingdomino/conv/${now.toISOString()}`;
